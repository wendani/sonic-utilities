...
def config(ports):
    ...
    poll_interval = configdb.get_entry( 'PFC_WD', 'GLOBAL').get('POLL_INTERVAL')
    if poll_interval is not None:
        click.echo("Changed polling interval to " + poll_interval + "ms")

    big_red_switch = configdb.get_entry( 'PFC_WD', 'GLOBAL').get('BIG_RED_SWITCH')
...
def start(action, restoration_time, ports, detection_time):
    ...
    for port in ports:
        if port == "all":
            for p in all_ports:
                configdb.mod_entry("PFC_WD", p, None)
                configdb.mod_entry("PFC_WD", p, pfcwd_info)
        else:
            if port not in all_ports:
                continue
            configdb.mod_entry("PFC_WD", port, None)
            configdb.mod_entry("PFC_WD", port, pfcwd_info)
...
def interval(poll_interval):
    ...
    configdb.mod_entry("PFC_WD", "GLOBAL", pfcwd_info)
...
def start_default():
    ...
    for port in active_ports:
        configdb.set_entry("PFC_WD", port, pfcwd_info)
